...
load("my_model.rda")
summary (m1)
A1 <- read.table ("summerize.human.ORFs.txt", sep="\t", header=T)
fdr1 <- A1$fdr
...
out <- data.frame(A1, B1, prob=p1[,2])
write.table (out, "prob.summerize.human.ORFs.txt", sep="\t", row.names=F, quote=F)
